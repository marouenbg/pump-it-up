...
layout = Layout(
    title='Y_Raspberry Pi Streaming Sensor Data'    
)
...
print py.plot(fig, filename='Y_Raspberry Pi Streaming Example Values')

#readadc.initialize()

stream = py.Stream(stream_token3)
stream.open()

while True:
	#sensor_data = readadc.readadc(sensor_pin, readadc.PINS.SPICLK, readadc.PINS.SPIMOSI, readadc.PINS.SPIMISO, readadc.PINS.SPICS)
	x, y, z = accel.read()
	stream.write({'x':datetime.datetime.now(), 'y': y})
